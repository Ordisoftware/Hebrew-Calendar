...
/* try
  ...
  public partial class UndoRedoTextBox : TextBox
  {
    ...
    private void UndoRedoTextBox_KeyUp(object sender, KeyEventArgs e)
    {
      // todo if selectionstart && SelectionLength changed push undo
    }
    ...
    private void ActionUndo_Click(object sender, EventArgs e)
    {
      ...
      {
        textbox.SetTextMutex = true;
        textbox.Previous.Set(textbox.Text, textbox.Pos);
        textbox.RedoStack.Push(textbox.Previous);
        var item = textbox.UndoStack.Pop();
        textbox.Text = item.Text;
        textbox.SelectionStart = item.SelectionStart;
    ...
    private void ActionRedo_Click(object sender, EventArgs e)
    {
      ...
      {
        textbox.SetTextMutex = true;
        textbox.UndoStack.Push(new UndoRedoItem().Set(textbox.Text, textbox.Pos));
        var item = textbox.RedoStack.Pop();
        Text = item.Text;
        textbox.SelectionStart = item.SelectionStart;
        textbox.Pos = textbox.SelectionStart;
        textbox.Previous.Set(textbox.Text, textbox.Pos);
        if ( textbox.Multiline ) textbox.ScrollToCaret();
      }
      finally
