...
static partial class AppTranslations
{
  ...
  static public readonly TranslationsDictionary NoticeMonthsAndDays = new()
  {
    ...
    [Language.FR] = """
                    Les dates générées avec le omer strict de la lune ou du soleil peuvent différer de celles du calendrier des autorités hébraïques, mais astronomiquement parlant, il n'y a pas d'erreur, à moins d'un problème à corriger.

                    Le logiciel calcule en effet les jours selon la lune ou le soleil et divers paramètres relatifs aux astres et aux saisons qui font que les dates peuvent parfois différer en certains points du calendrier établit selon les normes traditionnelles.

                    Ainsi le logiciel peut indiquer parfois les mêmes jours que le calendrier officiel, et parfois un à deux ou trois jours de décalage, en raison de la date de nouvelle lune qui n'est parfois pas en correspondance astronomique, et il en va de même pour le calcul des fêtes.

                    Il peut également arriver que sur 10 ou 20 ans il y ait un décalage d'un mois à cause du Adar II en raison de la règle de calcul utilisée pour déterminer le 1er Nissan et Pessa'h qui est que Pessa'h est le 14ème jour du premier mois saisonnier en première pleine lune qui suit l'équinoxe de printemps.

                    Mais même avec ces décalages, Pessa'h, selon le logiciel, est toujours la première nouvelle lune qui suit l'équinoxe de printemps, et en cas de décalage il tombe au pire en tant que Pessa'h Sheni du calendrier traditionnel, ou vice versa, et à quelques heures près qui restent généralement dans les 3% de marge universelle.
  ...
  static public readonly TranslationsDictionary NoticeCelebrations = new()
  {
    ...
    [Language.FR] = """
                    Les temps de célébration de la Torah sont Pessa'h ou Pâques qui est la libération des illusions, Shavouh'ot ou Semaines qui est le don de la connaissance, Terouh'ah ou Sonnerie qui est la joie d'être libéré, Kipourim ou Pénitence qui est les peines des pertes, et Soukot ou Cabanes qui est la construction du futur.
                    ...
                    L'application utilise par défaut un omer selon la lune pour les célébrations.
  ...
  static public readonly TranslationsDictionary NoticeCelebrationsFood = new()
  {
    ...
    [Language.FR] = """
                    ...
                    • Diète sans céréales ni graines ni fruits ni légumes du verger entre la fin du premier Omer de sept semaines et le lendemain du Shabat suivant.
  ...
  {
    [Language.EN] = """
                    Use the following tags to replace values:
    ...
    [Language.FR] = """
                    Utiliser les tags suivants pour remplacer les valeurs :
  ...
  static public readonly TranslationsDictionary NoticeDatesDiff = new()
  {
    [Language.EN] = """
                    ...
                    The annual solar period begins on January 1.
                    The monthly solar period is counted from the 1st day.
                    The solar week is counted every 7 days from the smallest date.

                    The lunar day goes from the setting of the moon to the next.
                    A lunar month goes from the new moon to the next.

                    Here are some examples.

                    Two solar weeks indicate that 8 days or more but 14 or less have passed.

                    Two solar months indicate that the oldest date is in a calendar month and the other is in the following month, regardless of the number of days between, i.e. the selected dates are at horse over two months.

                    Two lunar months indicate that one moon has elapsed and that we are in the second.
    ...
    [Language.FR] = """
                    Les valeurs indiquées incluent les périodes en cours.

                    La période solaire annuelle commence le 1er janvier.
                    La période solaire mensuelle est comptée depuis le 1er jour.
                    La semaine solaire est comptée tous les 7 jours depuis la plus petite date.

                    Le jour lunaire va du coucher de la lune au suivant.
                    Un mois lunaire va de la nouvelle lune à la suivante.

                    Voici quelques exemples.

                    Deux semaines solaires indiquent qu'il s'est écoulé 8 jours ou plus mais 14 ou moins.

                    Deux mois solaires indiquent que la date la plus ancienne se trouve dans un mois calendaire et que l'autre se trouve dans le mois suivant, quel que soient le nombre de jours entre, c'est-à-dire que les dates sélectionnées sont à cheval sur deux mois.

                    Deux mois lunaires indiquent qu'il s'est écoulé une lune et qu'on se trouve dans la deuxième.
