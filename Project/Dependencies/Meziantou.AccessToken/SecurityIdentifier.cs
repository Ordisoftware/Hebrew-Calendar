...
public sealed class SecurityIdentifier : IEquatable<SecurityIdentifier>
{
  ...
  public static SecurityIdentifier FromWellKnown(WellKnownSidType type)
  {
    ...
    try
    {
      if ( !Natives.NativeMethods.CreateWellKnownSid((int)type, IntPtr.Zero, resultSid, ref size) )
  ...
  {
    if ( Natives.NativeMethods.ConvertSidToStringSidW(sid, out var result) )
      return result;
  ...
  private static void LookupName(IntPtr sid, out string domain, out string name)
  {
    ...
    if ( Natives.NativeMethods.LookupAccountSidW(systemName: null, sid, bufUserName, ref userNameLen, bufDomainName, ref domainNameLen, ref sidNameUse) != 0 )
    {
    ...
    if ( error == Natives.NativeMethods.ERROR_NONE_MAPPED )
    {
      domain = default;
      name = default;
      return;
  ...
  {
    return other is not null && Sid == other.Sid;
  }
