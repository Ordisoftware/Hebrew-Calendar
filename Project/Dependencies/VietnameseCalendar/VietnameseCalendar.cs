...
namespace System.Globalization
{
  ...
  public class VietnameseCalendar : Calendar
  {
    ...
    public override int TwoDigitYearMax
    {
      get
      {
        if ( base.TwoDigitYearMax < 0 )
        {
          ...
          base.TwoDigitYearMax = GetSystemTwoDigitYearSetting(1/*CAL_GREGORIAN - Gregorian (localized)*/, GetYear(maxDate));
        }
    ...
    internal static int GetTwoDigitYearMax(int calendarId)
    {
      try
      {
        int num = 0, ret = NativeMethods.GetCalendarInfo(0x042a/*vi-VN*/,
                                                         calendarId,
    ...
    public override int GetMonth(DateTime time)
    {
      ...
      while ( date < time ) date = date.AddDays(GetMonthLength(year, month++, leapMonth));
      // DEBUG
...
#pragma warning restore S2589 // Boolean expressions should not be gratuitous
    ...
    public virtual string GetPropitiousHour(DateTime date)
    {
      ...
      for ( int i = 0; i < 12; i++ )
      {
        if ( gioHD[i] != '0' )
        {
          ret.Append(terrestrialBranches[i]);
          ret.Append(" (").Append(( i * 2 + 23 ) % 24).Append('-').Append(( i * 2 + 1 ) % 24).Append(')');
